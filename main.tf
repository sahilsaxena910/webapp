...
resource "aws_launch_template" "web_launch_template" {
  ...
  user_data              = var.isDocker? base64encode(file("${path.module}/user_data_ansible.sh")) :base64encode(file("${path.module}/user_data.sh")) ## base64 encoding of user data script is required by aws
